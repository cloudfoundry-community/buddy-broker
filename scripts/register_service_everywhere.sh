...
# USAGE: ORG=dingotiles ./scripts/register_service_everywhere.sh dingo-s3 username password baseurl
...
if [[ "${ORG}X" == "X" ]]; then
  # Iterate thru all orgs
  orgs_next_page_url="/v2/organizations"
else
  orgs_next_page_url="/v2/organizations?q=name:${ORG}"
fi
